...
\begin{align}
    z(\tau) & = \frac{1}{N - \tau} \sum_{i = 1}^{N-\tau} \bar{x(i)} \times x(i + \tau) \\
    & = \frac{1}{N-\tau} \sum_{i = 1}^{N-\tau} e^{-jwi} e^{jw(i+\tau)} \\
    & = \frac{1}{N-\tau} \sum_{i = 1}^{N-\tau} e^{jw\tau}
\end{align}
...
\begin{equation}
    z = \frac{1}{N} \sum_{i=1}^N Sine_1(i) * \tilde{Sine_2(i)}
\end{equation}
...
通过频偏估计得到补偿因子$\epsilon$，则对信号$S(k)$补偿的结果为，
...
从信道补偿之后的导频信号中截取m序列$\tilde{m(k)}$，并和已知原始信号$m(k)$进行信道估计，
