...
本章节主要介绍无线密钥生成系统的理论基础。无线密钥生成系统的本质是利用无线信道的短时互易性、时空唯一性进行密钥生成的工作。另外本章介绍了系统生成密钥的评估标准，通过CSI相关性、信道随机性、密钥随机性和信息泄漏率等五个方面评估系统生成密钥的可靠性和安全性。

\section{无线信道特征}
\section{信道估计}

在一个单入单出线性系统中，设脉冲响应函数为$g(k)$，根据维纳霍夫方程，发射信号$x(k))$和$y(k)$之间的相关函数为，
